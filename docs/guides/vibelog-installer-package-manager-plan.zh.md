...
Local installer scripts 现在处于 scratch rollback verified prototype 状态。公开 installer 命令会预览把 VibeLog skill、scripts、docs、README 和 package metadata 复制到用户选择的位置，但不会写入文件。
...
verifier 会把计划中的文件复制到临时 scratch target，确认安装形态，删除 target，并确认回滚移除了本次创建的内容。公开 installer 仍然拒绝 `--write`。

必要 gates：

- dry-run mode；
- scratch rollback verification；
- 写入已有 target 前需要 backup 或 restore verification；
...
- 没有 rollback verification 和明确批准前，不做 global installer。
- Markdown 继续是事实源。
- JSON 继续是导出格式。
...
最强的下一步是已有 target 的 backup/restore verification，或 release-bundle verification。用户可见的 installer write mode 仍然需要明确批准。

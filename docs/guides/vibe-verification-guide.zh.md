...
VibeLog 验证应该优先由 agent 执行。人类负责审查方向、真实性和最终证据；agent 负责运行可重复的检查，并把结果记录下来。

## 原则

如果 agent 可以 vibe 验证，就不要让人类手动验证。

人工审查应该聚焦判断，而不是重复劳动。agent 应该创建场景、运行命令、导出结构化数据、校验输出，并汇报证据。

## 人类角色

- 批准或拒绝产品方向。
- 拒绝看起来虚假、过度包装、或脱离真实工作的日志。
- 判断 handoff 状态是否能让另一个人或 agent 理解。
...
把 exporter、validator、drift check 和仓库测试套件放在一起运行。生成示例目录只能包含生成的 VibeLog 记录，不能包含 scratch 项目源码。

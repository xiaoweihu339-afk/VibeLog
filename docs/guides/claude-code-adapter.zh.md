...
当由 Claude Code hooks 调用时，省略 `--input`；adapter 会从 stdin 读取 hook JSON。

## 支持的 Hook Events

- `UserPromptSubmit`：把工程执行提示词记录为 `prompt_submitted`。
- `PostToolUse`：把测试命令记录为 `test_ran`；其他 tool use 记录为 `tool_used`。
...
- 本仓库中完成的是 fixture verification，不是真实 live-session verification。

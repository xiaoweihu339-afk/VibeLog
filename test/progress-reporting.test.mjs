...
test("progress reporting docs use the current S23 baseline", async () => {
  const english = await readFile(englishGuidePath, "utf8");
  const chinese = await readFile(chineseGuidePath, "utf8");

  assert.match(english, /Project Progress: 16 \/ 100/);
  assert.match(english, /S23 VibeLog self-update loop/);
  assert.match(english, /automatic continuous recording/i);
  ...
  assert.match(chinese, /项目总进度：16 \/ 100/);
  assert.match(chinese, /S23/);
  assert.match(chinese, /自更新闭环/);
...
test("agent-facing progress examples point to the current baseline", async () => {
  ...
  for (const content of [skill, agentGuide]) {
    assert.match(content, /Project Progress: 16 \/ 100/);
    assert.match(content, /S23/);

...
test("progress reporting docs use the current S24 baseline", async () => {
  const english = await readFile(englishGuidePath, "utf8");
  const chinese = await readFile(chineseGuidePath, "utf8");

  assert.match(english, /Project Progress: 18 \/ 100/);
  assert.match(english, /S24 local event stream recording/i);
  assert.match(english, /live hook environments/i);
  ...
  assert.match(chinese, /项目总进度：18 \/ 100/);
  assert.match(chinese, /S24/);
  assert.match(chinese, /本地 event stream 自动记录/);
...
test("agent-facing progress examples point to the current baseline", async () => {
  ...
  for (const content of [skill, agentGuide]) {
    assert.match(content, /Project Progress: 18 \/ 100/);
    assert.match(content, /S24/);
